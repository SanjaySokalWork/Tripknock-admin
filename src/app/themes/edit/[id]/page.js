...
export default function EditDestination({ params }) {
  ...
  // Fetch tour options
  useEffect(() => {
    document.title = 'Edit Theme Page - Tripknock';
    setLoading('fetchData', true, 'Loading theme page data...');
    let mockTours = [
    ...
    const fetchTours = async () => {
      try {
        let res = await fetch("https://data.tripknock.in/themes-pages/get/" + id);
        res = await res.json();
        let data = {
          ...
          popularPackages: res.popularDestinations,
          mainPackages: res.mainPackages,
        ...
        showError('Failed to load theme page data');
      } finally {
  ...
  async function validateSlug(slug) {
    let res = await fetch(`https://data.tripknock.in/themes-pages/validate`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleSubmit = async (status) => {
    showGlobalLoading('Updating theme page...');
    ...
    try {
      ...
      let res = await fetch("https://data.tripknock.in/themes-pages/update/" + id, {
        method: 'POST',
        headers: {
      ...
      res = await res.json();
      if (res.status === true) {
        showSuccess('Theme page updated successfully!');
        setTimeout(() => {
          router.push('/themes');
        }, 1500);
      } else {
        showError(res.message || 'Failed to update theme page');
      }
    } catch (error) {
      console.log('Error creating destination:', error);
      showError('An error occurred while updating theme page');
    } finally {
      hideGlobalLoading();
    }
  };
      ...
      {isLoading('fetchData') ? <Loading /> :
        <div ref={sectionRef} className="container mx-auto px-4 py-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-2xl font-bold text-secondary-900">Edit Theme Page</h1>
              <p className="text-secondary-600">Edit theme page</p>
            </div>
