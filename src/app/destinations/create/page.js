...
export default function CreateDestination() {
  ...
  // Fetch tour options
  useEffect(() => {
    document.title = 'Create Destination - Tripknock';
  ...
  async function validateType(type) {
    ...
    const data = await response.json();
    if (data.status !== true) {
      showError('Destination already exist');
    }
  }
  ...
  const handleSubmit = async (status) => {
    ...
    if (formData.destinationName === "") {
      ...
      showError('Please fill Destination Name');
      setSaving(false);
      return;
    }
    ...
    if (status === 'published' && formData.images.length === 0) {
      setActiveTab('basic');
      showError('At least one image is required for published destinations');
      setSaving(false);
      return;
    }
    ...
    try {
      ...
      if (res.status === true) {
        ...
        if (res.validationErrors && res.validationErrors.length > 0) {
          ...
          showSuccess(`Destination ${status === 'published' ? 'published' : 'saved'} successfully!`);
          router.push('/destinations');
        }
      } else {
        showError(res.message || 'Failed to save destination');
      }
    } catch (error) {
      console.error('Error:', error);
      showError('An error occurred while saving the destination');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div ref={sectionRef} className="container mx-auto px-4 py-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-secondary-900">Create Destination</h1>
          <p className="text-secondary-600">Add a new destination</p>
        </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="mt-1 text-xs text-secondary-600">
                  Selected: {formData.popularPackages.length}/4 minimum
                  {formData.popularPackages.length < 4 && (
                    <span className="text-red-500 ml-2">
                      • {4 - formData.popularPackages.length} more needed for published status
                    </span>
                  )}
                </div>
              </div>
              <div>
                ...
                />
                <div className="mt-1 text-xs text-secondary-600">
                  Selected: {formData.mainPackages.length}/1 minimum
                  {formData.mainPackages.length < 1 && (
                    <span className="text-red-500 ml-2">
