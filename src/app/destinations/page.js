...
export default function DestinationsPage() {
  ...
  useEffect(() => {
    document.title = 'Destinations - Tripknock';
    ...
    async function fetchAll() {
      try {
        ...
        const uniqueNames = [...new Set(data.map(dest => dest.title))].sort();
        setUniqueDestinations(uniqueNames);
      } catch (error) {
        console.log('Error fetching destinations:', error);
        showError('Failed to load destinations');
      } finally {
        setLoading('fetchDestinations', false);
      }
    }
    fetchAll();
  }, [change]);
  ...
  const filteredDestinations = destinations.filter((destination) => {
    ...
    const matchesDestination =
      ...
      destination.title === destinationFilter;

    // Search filter (name, title, country, id)
    const matchesSearch =
      destination.title?.toLowerCase().includes(searchTerm) ||
      destination.destination?.toLowerCase().includes(searchTerm) ||
      destination.country?.toLowerCase().includes(searchTerm) ||
      String(destination.id).includes(searchTerm.replace(/TKDES/gi, '').trim()); // For ID search
  ...
  const confirmDelete = async (id) => {
    ...
    try {
      ...
      if (data.status === false) {
        showError('An error occurred while deleting destination');
        return;
      } else {
      ...
      setLoading('deleteDestination', false);
      setDeleteItem(null);
      setChange(change + 1);
    }
  };
  ...
  return (
    <>
      {isLoading('fetchDestinations') ? <LoadingSpinner /> :
        <div className="container mx-auto px-4 py-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-2xl font-bold text-secondary-900">Destinations</h1>
              <p className="text-secondary-600">Manage your travel destinations</p>
            </div>
            <Link
                  ...
                  {filteredDestinations.length > 0 ? (
                    filteredDestinations.map((destination) => (
                      <tr key={destination.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="font-medium text-secondary-900">{destination.destination}</div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-secondary-600">
                          <div className="flex items-center gap-2">
                            ...
                            <Link
                              href={`/destinations/edit/${destination.slug}`}
                              className="text-secondary-600 hover:text-secondary-900"
                            >
          ...
          <AlertDialog
            ...
            confirmLabel={isLoading('deleteDestination') ? "Deleting..." : "Delete"}
            confirmVariant="error"
            onConfirm={() => confirmDelete(deleteItem.id)}
            onCancel={() => {
              setDeleteItem(null);
            }}
            disabled={isLoading('deleteDestination')}
          />
