...
export default function TourSeasons() {
  ...
  useEffect(() => {
    document.title = 'Tour Seasons - Admin Panel';
    fetchSeasons();
  }, [change]);
  ...
  const fetchSeasons = async () => {
    try {
      ...
      if (data.status === false) {
        showError(data.message || 'Failed to fetch seasons');
      } else {
        setSeasons(data);
      }
    } catch (error) {
      console.log('Error fetching seasons:', error);
      showError('Error fetching seasons');
    } finally {
      setLoading('fetchSeasons', false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status === true) {
        setChange(change + 1);
        showSuccess(`Season ${editItem ? 'updated' : 'added'} successfully`);
        setShowModal(false);
      } else {
        showError(data.message || `Failed to ${editItem ? 'update' : 'add'} season`);
      }
    } catch (error) {
      console.log('Error submitting season:', error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (data.status === true) {
        setChange(change + 1);
        showSuccess('Season deleted successfully');
      } else {
        showError(data.message || 'Failed to delete season');
      }
    } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-secondary-900">Tour seasons</h1>
        <button
          onClick={handleAdd}
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
            <div className="flex justify-between items-center p-4 border-b border-secondary-200">
              <h2 className="text-lg font-semibold text-secondary-900">
                {editItem ? 'Edit Season' : 'Add New Season'}
              </h2>
              <button
              ...
              <div className="flex justify-end space-x-3 pt-4">
                ...
                >
                  {isSubmitting ? (
                    ...
                    editItem ? 'Save Changes' : 'Add Season'
                  )}
                </button>
              </div>
            </form>
